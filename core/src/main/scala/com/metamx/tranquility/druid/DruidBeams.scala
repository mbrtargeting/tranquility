...
object DruidBeams
{
  ...
  ): Builder[MessageType] =
  {
    ...
    builder(timeFnFn(timestampSpec))
      ...
      .druidTuning(
        Option(specMap.getOrElse("tuningConfig", null)) map { tuningMap =>
          DruidTuning.fromMap(dict(tuningMap))
        } getOrElse {
          DruidTuning()
        }
      )
  ...
  class Builder[EventType] private[druid](private[tranquility] val config: BuilderConfig[EventType])
  {
    ...
    def ignoreLatestCloseTime() = {
      new Builder[EventType](config.copy(_ignoreLatestCloseTime = Some(true)))
    }
